		...
		for(var i=0; i<threePointersData.length; i++){
			...
			for(var j=0; j<timelineItems.length; j++){
				if(timelineItems[j].key==="issue.change_pipeline" 
					 && timelineItems[j].data.to_pipeline.name==="In Progress" 
					 && timelineItems[j].data.workspace.name==="Data Integration Pod" 
					 && new Date(threePointersData[i].closedAt)>=new Date("2023-02-01T00:00:00Z")){
					cycleTimes.push({
